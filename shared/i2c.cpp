...
void myI2C::Send_I2C_2Bytes(unsigned char DEVICE_ADDR, unsigned char Reg_ADDR, unsigned char Data0, unsigned char Data1){
	i2cSetAddress(DEVICE_ADDR);
	//cout << "beagle-i2c writing 0x"<< hex << (int)Data <<" to 0x"<<hex <<(int)DEVICE_ADDR << ", reg 0x" <<hex<<(int)Reg_ADDR <<"... ";
	I2C_WR_Buf[0] = Reg_ADDR;
	I2C_WR_Buf[1] = Data0;
	...
	if(write(g_i2cFile, I2C_WR_Buf, 3) != 3) {
		perror("Write Error in myI2C::Send_I2C_Byte");
	}
//	else cout << "OK";
...
unsigned char myI2C::Read_Multi_Byte(unsigned char DEVICE_ADDR, unsigned char Reg_ADDR, ssize_t n){
	I2C_WR_Buf[0] = Reg_ADDR;
