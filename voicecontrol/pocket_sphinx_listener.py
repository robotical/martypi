...
class PocketSphinxListener(object):
    ...
    def getCommand(self, debug=True):
        ...
        self.stream = self.pyAudio.open(input_device_index=2,format=pyaudio.paInt16,
                                       channels=1,
                                       rate=48000,      # prev 16000
                                       input=True,
        ...
        while True:
            try:
                ...
                # so, pocketsphinx's recognition is all based on 16kHz or 8kHz sampling
                # but we can't natively sample that low, apparently
                # sample rate is currently set to 48kHz, so for some ghetto downsampling to 16kHz we just need to extract every third sample
                # the soundbite is a string representing a series of 16-bit integers (see format in pyaudio.open)
                # 512 samples = string with length 1024
                # iterate through, grabbing every third pair of bytes:
                newsoundbite = ""
                for i in range(len(soundBite)/6 - 1):
                    newsoundbite += soundBite[i*6]
                    newsoundbite += soundBite[i*6+1]
                soundBite = newsoundbite
